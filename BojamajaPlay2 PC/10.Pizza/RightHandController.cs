...
namespace Pizza
{
    public class RightHandController : MonoBehaviour
    {
        ...
        public static RightHandController Instance { private set; get; }
        ...
        private void OnTriggerExit(Collider other)
        {
            if (isActivate && other.CompareTag("Dough") && AppManager.Instance.gameRunning)
            {
                ...
                {
                    LeftHandController.Instance.sphereCollider.enabled = false;
                }
                // 쥐고있을 때는 하나만 올라와야함
                StartNewPizza();
            }
        ...
        public void PinchDeActivate(bool isDeActivate)
        {
            ...
            if (leftHand.activeSelf)
            {
                LeftHandController.Instance.sphereCollider.enabled = true;
            }
        }
        ...
        private IEnumerator _StartNewPizza()
        {
            ...
            while (isActivate && AppManager.Instance.gameRunning && buildUp < .9999f)
            {
                ...
                currentPizza.transform.Rotate(0, (handDelta * 0.5f) + (100f * Time.deltaTime), 0f, Space.World); //피자 회전 Rotate 회전값
